...
public class MineStoreDependencies {
            ...
            for (MineStorePluginRepository repository : repositories) {
                if (file.exists()) {
                    if (dependency.verify(file, repository)) {
                        System.out.println("Dependency " + dependency.getName() + " with version " + dependency.getVersion() + " already downloaded from " + repository.getName());
                        found = true;
                        used.add(file);
                        break;
                    }
    ...
    public List<URI> getDependencyJars() {
